interface StatusBarProps {
  ...
  paired?: boolean;
  bridgeRung?: 'webrtc' | 'mailbox' | 'offline';
}
 ...
 */
export function StatusBar({ online, synced, ideaCount, paired, bridgeRung }: StatusBarProps) {
  return (
    <div className="flex-none flex items-center justify-between px-4 py-1.5 bg-ario-card/30 border-b border-white/5 gap-4">
      <div className="flex items-center gap-4 min-w-0">
        ...
        {bridgeRung && bridgeRung !== 'offline' && (
          <div className="flex items-center gap-2">
            <div
              className={`w-2.5 h-2.5 rounded-full flex-none ${
                bridgeRung === 'webrtc' ? 'bg-ario-turquoise' : 'bg-amber-400'
              }`}
              title={bridgeRung === 'webrtc' ? 'WebRTC' : 'Mailbox'}
              aria-label={bridgeRung === 'webrtc' ? 'WebRTC connection' : 'Mailbox connection'}
              role="img"
            />
          </div>
        )}
